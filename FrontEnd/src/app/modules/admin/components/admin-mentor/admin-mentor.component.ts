...
counts: number[] = [];
bill: Bill[]=[];
formGroup1: FormGroup<any> = new FormGroup({
  id_menu2: new FormControl(),
  name_menu2: new FormControl(),
  title_b2: new FormControl()
});
  ...
  save2() {
    const formData1 = {
      id_menu2: this.menn2.id_menu2,
      name_menu2: this.menn2.name_menu2,
      title_b2: this.menn2.title_b2,
      id_menu:this.menn2.id_menu,
      router_link_menu2:this.menn2.router_link_menu2
    ...
    this.httpClient.post(this.menuUrl, this.menuu).subscribe(data => {
      ...
      // Thực hiện các xử lý khác sau khi lưu thành công
      this.httpClient.post(this.menu2Url, formData1).subscribe(
        (data) => {
          console.log(data);
